...
if pyver < 37:
    print("*** ERROR *** This application needs Python 3.7 or higher.")
    sys.exit(1)
...
def k2_cadence_events(
    ...
    if from_archive:
        ...
        msg2 = (
            "The keyword target must be an integer or string (KIC/EPIC ID "
            + "or object name)"
        )
        msg3 = "The keyword cadence must be a string: 'short' or 'long'"
        msg4 = (
            "The keyword campaign must be an integer (a valid K2 Campaign " + "number)"
        )
        ...
        if not useTPF:
            try:
                # BUG DEPRECATED CODE FOUND WITH lightkurve >= 2.0.11
                # In lightkurve 2.0.11,
                # lk.search_lightcurvefile()
                # is just a wrapper routine for
                # lk.search_lightcurve()
                # BUG FIX: BEGIN
                # objf = lk.search_lightcurvefile(\  # DEPRECATED CODE
                assert lkver >= 200
                objf = lk.search_lightcurve(
                    target=target, cadence=cadence, campaign=campaign
            ...
            try:
                objf = lk.search_targetpixelfile(
                    target=target, cadence=cadence, campaign=campaign
                ).download(quality_bitmask=0)
            except Exception as e:
        ...
        assert filename is not None, "***** ERROR ***** A filename must be given"
        ...
        if not ok:
            try:
                ...
                print("[3] Exception raised: {}".format(e))
                sys.stdout.flush()
                sys.exit(1)
    ...
    try:
        ...
        filename = objf.filename  # use the "filename" attribute instead
    # BUG FIX: END
    ...
    try:
        # telescop = objf.hdu[0].header['TELESCOP']  # < lk_v2.0.11
    ...
    if telescop == "Kepler":
        ...
        assert tessData
    #
    # extname = objf.hdu[1].header['EXTNAME']  # < lk_v2.0.11
    extname = hdu[1].header["EXTNAME"]
    if isLCF:
    ...
    if keplerData:
        try:
            # obsmode = objf.hdu[0].header['OBSMODE']  # < lk_v2.0.11
            obsmode = hdu[0].header["OBSMODE"]
        except:
            obsmode = ""
    try:
        # mission = objf.hdu[0].header['MISSION']  # < lk_v2.0.11
        mission = hdu[0].header["MISSION"]
    except:
        mission = None
    ...
    # cadenceno = objf.hdu[1].data['CADENCENO'].copy()  # < lk_v2.0.11
    cadenceno = hdu[1].data["CADENCENO"].copy()
    if SAP_FLUX:
        flux_type = "SAP_FLUX"
    else:
        flux_type = "PDCSAP_FLUX"
    if isLCF:
        # flux = lcf.hdu[1].data[flux_type].copy()  # < lk_v2.0.11
        flux = hdu[1].data[flux_type].copy()
        if keplerData:
            # quality = lcf.hdu[1].data['SAP_QUALITY'].copy()  # < lk_v2.0.11
            quality = hdu[1].data["SAP_QUALITY"].copy()
        else:
            # quality = lcf.hdu[1].data['QUALITY'].copy()  # < lk_v2.0.11
            quality = hdu[1].data["QUALITY"].copy()
    else:
        flux = tpf.to_lightcurve().flux.copy()
        # quality = tpf.hdu[1].data['QUALITY'].copy()  # < lk_v2.0.111
        quality = hdu[1].data["QUALITY"].copy()
        flux_type = "SAP_FLUX"
    ...
    #
    try:
        # campaign = objf.hdu[0].header['CAMPAIGN']  # < lk_2.0.11
        campaign = hdu[0].header["CAMPAIGN"]
        print("K2 Campaign %d\n" % campaign)
    except:
        campaign = None
    try:
        # quarter = objf.hdu[0].header['QUARTER']  # < lk_2.0.11
        quarter = hdu[0].header["QUARTER"]
        print("Kepler Quarter %d\n" % quarter)
    except:
        quarter = None
    try:
        # sector = objf.hdu[0].header['SECTOR']  # < lk_2.0.11
        sector = hdu[0].header["SECTOR"]
        print("TESS Sector %d\n" % sector)
    except:
        sector = None
    # bjdrefi = objf.hdu[1].header['BJDREFI']  # < lk_v2.0.11
    bjdrefi = hdu[1].header["BJDREFI"]
    # object = objf.hdu[0].header['OBJECT']  # < lk_v2.0.11
    ...
    if report:
        if report_filename is not None:
            ...
            f.write("#\n")
            f.write(
                "# %d events with bitmask value of %d (= %s)\n"
                % (n_events, mask, mask_str)
            )
            if n_events > 0:
                f.write("#\n")
                f.write(
                    "#  Event         Time  Normalized_Flux  CADENCENO  "
                    + "===========QUALITY============\n"
                )
                for j, (xxx_, yyy_, ccc_, qqq_) in enumerate(zip(xxx, yyy, ccc, qqq)):
                    if keplerData:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=None,
        help="User-defined right and left limits for the X axis.\n"
    ...
    parser.add_argument(
        ...
        type=ast.literal_eval,
        default=None,
        help="User-defined bottom and top limits for the Y axis.\n"
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Filename of the report (if any) [default: None]",
    )
    parser.add_argument(
        "--n_before",
        action="store",
    ...
    parser.add_argument(
        ...
        type=ast.literal_eval,
        default=None,
        help="Cadences with time (X axis) values within the "
    ...
    parser.add_argument(
        ...
        type=ast.literal_eval,
        default=None,
        help="Cadences with normalized flux (Y axis) values "
    ...
    #
    _, n_events, _, idx_ = k2_cadence_events(
